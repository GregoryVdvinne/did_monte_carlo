...
my_event_plot <-  function(data) {
  data %>%
    ...
    ggplot(aes(x=term, y=estimate, color = estimator, shape = estimator)) + 
      ...
      geom_line(aes(x = term, y=true_effect, linetype = "solid"), color = myPal[1], linewidth = 0.75) + #True effect as a line
...
```
Above, we see the well-established result that TWFE yields biased estimates when treatment effects are not homogeneous. This issue is solved by the methods of Callaway and Sant'Anna (2021) and @gardner2022two. 
...
In this document, we have explored how the TWFE, @gardner2022two, and Callaway and Sant'Anna (2021) estimators behave (or misbehave) in a variety of circumstances. First, we showed that when treatment effects are heterogeneous, TWFE will produce biased estimates, while the other two estimators can produce unbiased estimates. Next, we illustrated that if covariates exist that have fixed values but still effect the dynamics of the dependent variable (ie, their effects change over time), both @gardner2022two and TWFE will produce biased estimates, while the Callaway and Sant'Anna (2021) estimator can produce unbiased estimates if we condition on those covariates. Last, we saw that @gardner2022two and TWFE can control for covariates whose values change over time, while Callaway and Sant'Anna (2021) cannot. Of course, TWFE still requires homogeneous effects in this scenario.

Clearly, I have brushed over multiple assumptions and potential pitfalls with each of these estimators. The purpose of this was not to give a comprehensive overview of these methods, but rather to give a glimpse of some of the practical differences between them and some basic examples of scenarios in which one or two of the methods might be more appropriate than the other(s). For a more comprehensive understanding of these methods I suggest not only reading the @gardner2022two and Callaway and Sant'Anna (2021) papers, but also looking at some of the documentation for the {did} and {did2s} R packages. The vignette https://cran.r-project.org/web/packages/did/vignettes/TWFE.html, for example, gives a very approachable overview of some issues with TWFE estimators and how Callaway and Sant'Anna remedy such problems with their estimator. I also recommend Dr. Scott Cunningham's Substack (https://causalinf.substack.com/) for discussions about all things causal inference. 
